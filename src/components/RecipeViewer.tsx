...
export const RecipeViewer: React.FC<RecipeViewerProps> = ({ recipe, requiredQuantity = recipe.output.quantity }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3 text-xs">
      ...
      <div className="flex items-center justify-center gap-4 py-1">
        ...
        <div>
          {isSmelting ? (
            ...
            <div
              className={`grid gap-1 p-1.5 bg-slate-200 rounded-lg border border-slate-300 ${
            ...
            <div className="space-y-1">
              {craftDetails.ingredientsNeeded.map((ing) => (
                <div
                  key={ing.itemId}
                  className="flex items-center justify-between gap-3 px-2 py-1 bg-white rounded border border-slate-200"
                >
      ...
      {/* Production stats */}
      <div className="grid grid-cols-3 gap-2 p-2.5 bg-white rounded-lg border border-slate-200 text-center font-mono text-[11px]">
        <div>
          ...
          {craftDetails.ingredientsNeeded.map((ing) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-800">{ing.displayName}</span>
              </div>
