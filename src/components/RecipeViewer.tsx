...
export const RecipeViewer: React.FC<RecipeViewerProps> = ({
  ...
  const getRecipeTypeLabel = (r: Recipe) => {
    if (r.type === 'stonecutting') return { label: 'Stonecutter', icon: '🪚', efficiency: '⭐ Most efficient' };
    if (r.type === 'smelting') return { label: 'Furnace', icon: '🔥', efficiency: '' };
    if (r.type === 'blasting') return { label: 'Blast Furnace', icon: '💨', efficiency: '2x Faster' };
    if (r.type === 'smoking') return { label: 'Smoker', icon: '🍖', efficiency: '2x Faster' };
    if (r.type === 'crafting_shaped') return { label: `Crafting Table (${r.gridSize || '3x3'})`, icon: '🛠️', efficiency: '' };
    if (r.id.includes('from_cobblestone_and_quartz') || r.id.includes('from_diorite')) {
      return { label: 'Synthesis (Quartz)', icon: '⚗️', efficiency: 'Manual crafting' };
    }
    return { label: 'Crafting Table', icon: '🛠️', efficiency: '' };
  };
    ...
    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3.5 text-xs">
        ...
        <div className="space-y-1.5 pb-1 border-b border-slate-200">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
              ...
              Available Methods ({recipesList.length})
            </span>
          </div>
        ...
        <div className="flex items-center gap-1.5 font-semibold text-slate-700">
          {isStonecutting && (
            <span className="inline-flex items-center gap-1 text-emerald-700 font-bold">
              <Scissors className="w-3.5 h-3.5" /> Stonecutter
            </span>
          )}
          {isSmelting && (
            <span className="inline-flex items-center gap-1 text-orange-700 font-bold">
              <Flame className="w-3.5 h-3.5" /> Furnace Smelting
            </span>
          )}
          {!isStonecutting && !isSmelting && (
            <span className="inline-flex items-center gap-1 text-slate-800">
              <Hammer className="w-3.5 h-3.5" /> Crafting Grid ({currentRecipe.gridSize || '3x3'})
            </span>
          )}
        </div>
            ...
            <div className="p-2.5 bg-white rounded-lg border border-slate-200 flex items-center gap-2 shadow-2xs">
              ...
              <div>
                ...
                <span className="text-[11px] font-mono text-slate-500">
                  {craftDetails.ingredientsNeeded[0]?.quantity} items
                </span>
              </div>
            </div>
        ...
        <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-center gap-2.5 shadow-2xs">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-400 font-mono">
              ({currentRecipe.output.quantity}x per craft)
            </div>
          </div>
        </div>
      </div>
      ...
      {isStonecutting && (
        <div className="p-2 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-800 text-[11px] flex items-center gap-1.5">
          ...
          <span>
            <b>High Efficiency:</b> The stonecutter produces 1 block/stairs directly without recipe ingredient waste.
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2 p-2.5 bg-white rounded-lg border border-slate-200 text-center font-mono text-[11px] shadow-2xs">
        ...
        <div>
          <span className="text-slate-400 block text-[10px]">Surplus</span>
      ...
      <div className="space-y-1 pt-1">
        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
          Ingredients needed for {craftDetails.craftsRequired} {craftDetails.craftsRequired === 1 ? 'craft' : 'crafts'}:
        </span>
