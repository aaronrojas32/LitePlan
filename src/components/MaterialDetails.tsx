...
export const MaterialDetails: React.FC<MaterialDetailsProps> = ({ material, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end bg-slate-900/40 backdrop-blur-xs transition-opacity text-xs">
      ...
      >
        {/* Header */}
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-4 sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-10">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center p-1 shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 transition cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-850 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1.5 shadow-2xs">
            ...
            <div className="font-mono text-emerald-700 dark:text-emerald-400 text-xs font-bold pt-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 bg-slate-50 dark:bg-slate-850 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <span className="text-lg font-bold text-slate-900 dark:text-slate-100 font-mono mt-0.5 block">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-850 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <span className={`text-lg font-bold font-mono mt-0.5 block ${material.missing > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
            ...
            <div className="space-y-4 pt-2">
              <div className="flex border-b border-slate-200 dark:border-slate-800 gap-4">
                <button
                  ...
                  className={`pb-2.5 font-semibold flex items-center gap-1.5 border-b-2 transition ${
                    activeTab === 'recipe'
              ...
              {activeTab === 'raw' && (
                <div className="space-y-2">
                  {rawBreakdown.rawMaterials.map((raw) => (
                    <div
                      key={raw.itemId}
                      className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-850 rounded-lg border border-slate-200 dark:border-slate-800"
                    >
                      <div className="flex items-center gap-2.5">
                        <ItemIcon itemId={raw.itemId} size={22} />
                        <div>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{raw.displayName}</span>
                          <span className="text-[11px] text-slate-400 font-mono block">{raw.itemId}</span>
                        </div>
                      </div>
                      <div className="text-right font-mono">
                        <span className="font-bold text-slate-900 dark:text-slate-100">{raw.quantity.toLocaleString()}</span>
